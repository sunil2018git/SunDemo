 ...
 * found in the LICENSE file at https://angular.io/license
 */
function stringifyValueForError(value){return value&&value.native&&value.native.outerHTML?value.native.outerHTML:"string"==typeof value?'"'+value+'"':value}function assertEqual(actual,expected,name,serializer){actual!=expected&&assertThrow(actual,expected,name,"==",serializer)}function assertLessThan(actual,expected,name){actual>=expected&&assertThrow(actual,expected,name,"<")}function assertNotNull(actual,name){assertNotEqual(actual,null,name)}function assertNotEqual(actual,expected,name){actual==expected&&assertThrow(actual,expected,name,"!=")}function assertThrow(actual,expected,name,operator,serializer){void 0===serializer&&(serializer=stringifyValueForError);var error="ASSERT: expected "+name+" "+operator+" "+serializer(expected)+" but was "+serializer(actual)+"!";throw new Error(error)}/**
 * @license
 * Copyright Google Inc. All Rights Reserved.
 *
 * Use of this source code is governed by an MIT-style license that can be
 * found in the LICENSE file at https://angular.io/license
 */
function assertNodeType(node,type){assertNotEqual(node,null,"node"),assertEqual(3&node.flags,type,"Node.type",typeSerializer)}function assertNodeOfPossibleTypes(node){for(var types=[],_i=1;_i<arguments.length;_i++)types[_i-1]=arguments[_i];assertNotEqual(node,null,"node");for(var nodeType=3&node.flags,i=0;i<types.length;i++)if(nodeType===types[i])return;throw new Error("Expected node of possible types: "+types.map(typeSerializer).join(", ")+" but got "+typeSerializer(nodeType))}function typeSerializer(type){return 1==type?"Projection":0==type?"Container":2==type?"View":3==type?"Element":"??? "+type+" ???"}/**
 * @license
 * Copyright Google Inc. All Rights Reserved.
 *
 * Use of this source code is governed by an MIT-style license that can be
 ...
 * found in the LICENSE file at https://angular.io/license
 */
function isDifferent(a,b){return!(a!==a&&b!==b)&&a!==b}function stringify$1(value){return"function"==typeof value?value.name||value:"string"==typeof value?value:null==value?"":""+value}function notImplemented(){return new Error("NotImplemented")}function flatten$1(list){for(var result=[],i=0;i<list.length;){var item=list[i];Array.isArray(item)?item.length>0?(list=item.concat(list.slice(i+1)),i=0):i++:(result.push(item),i++)}return result}function enterView(newView,host){var oldView=currentView;return data=newView.data,bindingIndex=newView.bindingStartIndex||0,tData=newView.tView.data,creationMode=newView.creationMode,cleanup=newView.cleanup,renderer=newView.renderer,null!=host&&(previousOrParentNode=host,isParent=!0),currentView=newView,currentQueries=newView.queries,oldView}function leaveView(newView){executeHooks(currentView.data,currentView.tView.viewHooks,currentView.tView.viewCheckHooks,creationMode),currentView.creationMode=!1,currentView.lifecycleStage=1,currentView.tView.firstTemplatePass=!1,enterView(newView,null)}function createLView(viewId,renderer,tView,template,context){return void 0===template&&(template=null),void 0===context&&(context=null),{parent:currentView,id:viewId,node:null,data:[],tView:tView,cleanup:null,renderer:renderer,child:null,tail:null,next:null,bindingStartIndex:null,creationMode:!0,template:template,context:context,dynamicViewCount:0,lifecycleStage:1,queries:null}}function createLNode(index,type,native,state){var parent=isParent?previousOrParentNode:previousOrParentNode&&previousOrParentNode.parent,queries=(isParent?currentQueries:previousOrParentNode&&previousOrParentNode.queries)||parent&&parent.queries&&parent.queries.child(),isState=null!=state,node={flags:type,native:native,view:currentView,parent:parent,child:null,next:null,nodeInjector:parent?parent.nodeInjector:null,data:isState?state:null,queries:queries,tNode:null,pNextOrParent:null};return 2==(2&type)&&isState&&(ngDevMode&&assertEqual(state.node,null,"lView.node"),state.node=node),null!=index&&(ngDevMode&&assertDataNext(index),data[index]=node,index>=tData.length?tData[index]=null:node.tNode=tData[index],isParent?(currentQueries=null,previousOrParentNode.view!==currentView&&2!=(3&previousOrParentNode.flags)||(ngDevMode&&assertEqual(previousOrParentNode.child,null,"previousNode.child"),previousOrParentNode.child=node)):previousOrParentNode&&(ngDevMode&&assertEqual(previousOrParentNode.next,null,"previousNode.next"),previousOrParentNode.next=node)),previousOrParentNode=node,isParent=!0,node}function resetApplicationState(){isParent=!1,previousOrParentNode=null}function renderEmbeddedTemplate(viewNode,template,context,renderer){var _isParent=isParent,_previousOrParentNode=previousOrParentNode;try{isParent=!0,previousOrParentNode=null;var cm=!1;if(null==viewNode){viewNode=createLNode(null,2,null,createLView(-1,renderer,createTView(),template,context)),cm=!0}enterView(viewNode.data,viewNode),template(context,cm)}finally{refreshDynamicChildren(),leaveView(currentView.parent),isParent=_isParent,previousOrParentNode=_previousOrParentNode}return viewNode}function renderComponentOrTemplate(node,hostView,componentOrContext,template){var oldView=enterView(hostView,node);try{rendererFactory.begin&&rendererFactory.begin(),template?template(componentOrContext,creationMode):componentRefresh(1,0)}finally{rendererFactory.end&&rendererFactory.end(),leaveView(oldView)}}function elementStart(index,nameOrComponentType,attrs,directiveTypes,localRefs){var node,native;if(null==nameOrComponentType){var node_1=data[index];native=node_1&&node_1.native}else{ngDevMode&&assertEqual(currentView.bindingStartIndex,null,"bindingStartIndex");var isHostElement="string"!=typeof nameOrComponentType,hostComponentDef=isHostElement?nameOrComponentType.ngComponentDef:null,name_1=isHostElement?hostComponentDef.tag:nameOrComponentType;if(null===name_1)throw"for now name is required";native=renderer.createElement(name_1);var componentView=null;if(isHostElement){var tView=getOrCreateTView(hostComponentDef.template);componentView=addToViewTree(createLView(-1,rendererFactory.createRenderer(native,hostComponentDef.rendererType),tView))}node=createLNode(index,3,native,componentView);var queryName=hack_findQueryName(hostComponentDef,localRefs,"");null==node.tNode&&(ngDevMode&&assertDataInRange(index-1),node.tNode=tData[index]=createTNode(name_1,attrs||null,null,hostComponentDef?null:queryName)),attrs&&setUpAttributes(native,attrs),appendChild(node.parent,native,currentView),hostComponentDef&&directiveCreate(++index,hostComponentDef.n(),hostComponentDef,queryName),hack_declareDirectives(index,directiveTypes,localRefs)}return native}function hack_declareDirectives(index,directiveTypes,localRefs){if(directiveTypes)for(var i=0;i<directiveTypes.length;i++){var directiveType=directiveTypes[i],directiveDef=directiveType.ngDirectiveDef;directiveCreate(++index,directiveDef.n(),directiveDef,hack_findQueryName(directiveDef,localRefs))}}function hack_findQueryName(directiveDef,localRefs,defaultExport){var exportAs=directiveDef&&directiveDef.exportAs||defaultExport;if(null!=exportAs&&localRefs)for(var i=0;i<localRefs.length;i+=2){var local=localRefs[i],toExportAs=localRefs[1|i];if(toExportAs===exportAs||toExportAs===defaultExport)return local}return null}function getOrCreateTView(template){return template.ngPrivateData||(template.ngPrivateData=createTView())}function createTView(){return{data:[],firstTemplatePass:!0,initHooks:null,checkHooks:null,contentHooks:null,contentCheckHooks:null,viewHooks:null,viewCheckHooks:null,destroyHooks:null}}function setUpAttributes(native,attrs){ngDevMode&&assertEqual(attrs.length%2,0,"attrs.length % 2");for(var isProceduralRenderer=renderer.setAttribute,i=0;i<attrs.length;i+=2)isProceduralRenderer?renderer.setAttribute(native,attrs[i],attrs[1|i]):native.setAttribute(attrs[i],attrs[1|i])}function createError(text,token){return new Error("Renderer: "+text+" ["+stringify$1(token)+"]")}function locateHostElement(factory,elementOrSelector){ngDevMode&&assertDataInRange(-1),rendererFactory=factory;var defaultRenderer=factory.createRenderer(null,null),rNode="string"==typeof elementOrSelector?defaultRenderer.selectRootElement?defaultRenderer.selectRootElement(elementOrSelector):defaultRenderer.querySelector(elementOrSelector):elementOrSelector;if(ngDevMode&&!rNode)throw"string"==typeof elementOrSelector?createError("Host node with selector not found:",elementOrSelector):createError("Host node is required:",elementOrSelector);return rNode}function hostElement(rNode,def){resetApplicationState(),createLNode(0,3,rNode,createLView(-1,renderer,getOrCreateTView(def.template)))}function listener(eventName,listener,useCapture){void 0===useCapture&&(useCapture=!1),ngDevMode&&assertPreviousIsParent();var node=previousOrParentNode,native=node.native;if(renderer.listen){var cleanupFn=renderer.listen(native,eventName,listener);(cleanup||(cleanup=currentView.cleanup=[])).push(cleanupFn,null)}else native.addEventListener(eventName,listener,useCapture),(cleanup||(cleanup=currentView.cleanup=[])).push(eventName,native,listener,useCapture);var tNode=node.tNode;void 0===tNode.outputs&&(tNode.outputs=null,tNode=generatePropertyAliases(node.flags,tNode));var outputData,outputs=tNode.outputs;outputs&&(outputData=outputs[eventName])&&createOutput(outputData,listener)}function createOutput(outputs,listener){for(var i=0;i<outputs.length;i+=2){ngDevMode&&assertDataInRange(outputs[i]);var subscription=data[outputs[i]][outputs[1|i]].subscribe(listener);cleanup.push(subscription.unsubscribe,subscription)}}function elementEnd(){isParent?isParent=!1:(ngDevMode&&assertHasParent(),previousOrParentNode=previousOrParentNode.parent),ngDevMode&&assertNodeType(previousOrParentNode,3);var queries=previousOrParentNode.queries;queries&&queries.addNode(previousOrParentNode),queueLifecycleHooks(previousOrParentNode.flags,currentView)}function elementProperty(index,propName,value){if(value!==NO_CHANGE){var node=data[index],tNode=node.tNode;void 0===tNode.inputs&&(tNode.inputs=null,tNode=generatePropertyAliases(node.flags,tNode,!0));var dataValue,inputData=tNode.inputs;if(inputData&&(dataValue=inputData[propName]))setInputsForProperty(dataValue,value);else{var native=node.native;renderer.setProperty?renderer.setProperty(native,propName,value):native.setProperty?native.setProperty(propName,value):native[propName]=value}}}function createTNode(tagName,attrs,data,localName){return{tagName:tagName,attrs:attrs,localNames:localName?[localName,-1]:null,initialInputs:void 0,inputs:void 0,outputs:void 0,data:data}}function setInputsForProperty(inputs,value){for(var i=0;i<inputs.length;i+=2)ngDevMode&&assertDataInRange(inputs[i]),data[inputs[i]][inputs[1|i]]=value}function generatePropertyAliases(flags,tNode,isInputData){void 0===isInputData&&(isInputData=!1);for(var start=flags>>12,size=(4092&flags)>>2,i=start,ii=start+size;i<ii;i++){var directiveDef=tData[i],propertyAliasMap=isInputData?directiveDef.inputs:directiveDef.outputs;for(var publicName in propertyAliasMap)if(propertyAliasMap.hasOwnProperty(publicName)){var internalName=propertyAliasMap[publicName],staticDirData=isInputData?tNode.inputs||(tNode.inputs={}):tNode.outputs||(tNode.outputs={}),hasProperty=staticDirData.hasOwnProperty(publicName);hasProperty?staticDirData[publicName].push(i,internalName):staticDirData[publicName]=[i,internalName]}}return tNode}function elementStyle(index,styleName,value,suffix){if(value!==NO_CHANGE){var lElement=data[index];null==value?renderer.removeStyle?renderer.removeStyle(lElement.native,styleName,RendererStyleFlags3.DashCase):lElement.native.style.removeProperty(styleName):renderer.setStyle?renderer.setStyle(lElement.native,styleName,suffix?stringify$1(value)+suffix:stringify$1(value),RendererStyleFlags3.DashCase):lElement.native.style.setProperty(styleName,suffix?stringify$1(value)+suffix:stringify$1(value))}}function text(index,value){ngDevMode&&assertEqual(currentView.bindingStartIndex,null,"bindingStartIndex");var textNode=null!=value?renderer.createText?renderer.createText(stringify$1(value)):renderer.createTextNode(stringify$1(value)):null,node=createLNode(index,3,textNode);isParent=!1,appendChild(node.parent,textNode,currentView)}function textBinding(index,value){ngDevMode&&assertDataInRange(index);var existingNode=data[index];ngDevMode&&assertNotNull(existingNode,"existing node"),existingNode.native?value!==NO_CHANGE&&(renderer.setValue?renderer.setValue(existingNode.native,stringify$1(value)):existingNode.native.textContent=stringify$1(value)):(existingNode.native=renderer.createText?renderer.createText(stringify$1(value)):renderer.createTextNode(stringify$1(value)),insertChild(existingNode,currentView))}function directiveCreate(index,directive,directiveDef,queryName){var instance;ngDevMode&&assertEqual(currentView.bindingStartIndex,null,"bindingStartIndex"),ngDevMode&&assertPreviousIsParent();var flags=previousOrParentNode.flags;if(0==(4092&flags)?flags=index<<12|4|3&flags:flags+=4,previousOrParentNode.flags=flags,ngDevMode&&assertDataInRange(index-1),Object.defineProperty(directive,NG_HOST_SYMBOL,{enumerable:!1,value:previousOrParentNode}),data[index]=instance=directive,index>=tData.length&&(tData[index]=directiveDef,queryName)){ngDevMode&&assertNotNull(previousOrParentNode.tNode,"previousOrParentNode.tNode");var tNode_1=previousOrParentNode.tNode;(tNode_1.localNames||(tNode_1.localNames=[])).push(queryName,index)}var diPublic=directiveDef.diPublic;diPublic&&diPublic(directiveDef);var tNode=previousOrParentNode.tNode;return tNode&&tNode.attrs&&setInputsFromAttrs(instance,directiveDef.inputs,tNode),queueInitHooks(index,directiveDef.onInit,directiveDef.doCheck,currentView.tView),instance}function setInputsFromAttrs(instance,inputs,tNode){var directiveIndex=((4092&previousOrParentNode.flags)>>2)-1,initialInputData=tNode.initialInputs;(void 0===initialInputData||directiveIndex>=initialInputData.length)&&(initialInputData=generateInitialInputs(directiveIndex,inputs,tNode));var initialInputs=initialInputData[directiveIndex];if(initialInputs)for(var i=0;i<initialInputs.length;i+=2)instance[initialInputs[i]]=initialInputs[1|i]}function generateInitialInputs(directiveIndex,inputs,tNode){var initialInputData=tNode.initialInputs||(tNode.initialInputs=[]);initialInputData[directiveIndex]=null;for(var attrs=tNode.attrs,i=0;i<attrs.length;i+=2){var attrName=attrs[i],minifiedInputName=inputs[attrName];if(void 0!==minifiedInputName){(initialInputData[directiveIndex]||(initialInputData[directiveIndex]=[])).push(minifiedInputName,attrs[1|i])}}return initialInputData}function container(index,directiveTypes,template,tagName,attrs,localRefs){ngDevMode&&assertEqual(currentView.bindingStartIndex,null,"bindingStartIndex");var currentParent=isParent?previousOrParentNode:previousOrParentNode.parent;ngDevMode&&assertNotEqual(currentParent,null,"currentParent");var lContainer={views:[],nextIndex:0,renderParent:canInsertNativeNode(currentParent,currentView)?currentParent:null,template:null==template?null:template,next:null,parent:currentView,dynamicViewCount:0,queries:null},node=createLNode(index,0,void 0,lContainer);if(null==node.tNode){var queryName=hack_findQueryName(null,localRefs,"");node.tNode=tData[index]=createTNode(tagName||null,attrs||null,[],queryName||null)}addToViewTree(node.data),hack_declareDirectives(index,directiveTypes,localRefs),isParent=!1,ngDevMode&&assertNodeType(previousOrParentNode,0);var queries=node.queries;queries&&(queries.addNode(node),lContainer.queries=queries.container())}function containerRefreshStart(index){ngDevMode&&assertDataInRange(index),previousOrParentNode=data[index],ngDevMode&&assertNodeType(previousOrParentNode,0),isParent=!0,previousOrParentNode.data.nextIndex=0,ngDevMode&&assertEqual(void 0===previousOrParentNode.native,!0,"previousOrParentNode.native === undefined"),executeInitHooks(currentView,currentView.tView,creationMode)}function containerRefreshEnd(){isParent?isParent=!1:(ngDevMode&&assertNodeType(previousOrParentNode,2),ngDevMode&&assertHasParent(),previousOrParentNode=previousOrParentNode.parent),ngDevMode&&assertNodeType(previousOrParentNode,0);var container=previousOrParentNode;container.native=void 0,ngDevMode&&assertNodeType(container,0);for(var nextIndex=container.data.nextIndex;nextIndex<container.data.views.length;)removeView(container,nextIndex)}function refreshDynamicChildren(){for(var current=currentView.child;null!==current;current=current.next)if(0!==current.dynamicViewCount&&current.views)for(var container_1=current,i=0;i<container_1.views.length;i++){var view=container_1.views[i];renderEmbeddedTemplate(view,view.data.template,view.data.context,renderer)}}function viewStart(viewBlockId){var container=isParent?previousOrParentNode:previousOrParentNode.parent;ngDevMode&&assertNodeType(container,0);var lContainer=container.data,views=lContainer.views,existingView=!creationMode&&lContainer.nextIndex<views.length&&views[lContainer.nextIndex],viewUpdateMode=existingView&&viewBlockId===existingView.data.id;if(viewUpdateMode)previousOrParentNode=views[lContainer.nextIndex++],ngDevMode&&assertNodeType(previousOrParentNode,2),isParent=!0,enterView(existingView.data,previousOrParentNode);else{var newView=createLView(viewBlockId,renderer,getOrCreateEmbeddedTView(viewBlockId,container));lContainer.queries&&(newView.queries=lContainer.queries.enterView(lContainer.nextIndex)),enterView(newView,createLNode(null,2,null,newView)),lContainer.nextIndex++}return!viewUpdateMode}function getOrCreateEmbeddedTView(viewIndex,parent){ngDevMode&&assertNodeType(parent,0);var tContainer=parent.tNode.data;return(viewIndex>=tContainer.length||null==tContainer[viewIndex])&&(tContainer[viewIndex]=createTView()),tContainer[viewIndex]}function viewEnd(){isParent=!1;var viewNode=previousOrParentNode=currentView.node,container=previousOrParentNode.parent;if(container){ngDevMode&&assertNodeType(viewNode,2),ngDevMode&&assertNodeType(container,0);var containerState=container.data,previousView=containerState.nextIndex<=containerState.views.length?containerState.views[containerState.nextIndex-1]:null;(null==previousView||previousView.data.id!==viewNode.data.id)&&insertView(container,viewNode,containerState.nextIndex-1)}leaveView(currentView.parent),ngDevMode&&assertEqual(isParent,!1,"isParent"),ngDevMode&&assertNodeType(previousOrParentNode,2)}function componentRefresh(directiveIndex,elementIndex){executeInitHooks(currentView,currentView.tView,creationMode),executeContentHooks(currentView,currentView.tView,creationMode);var template=tData[directiveIndex].template;if(null!=template){ngDevMode&&assertDataInRange(elementIndex);var element=data[elementIndex];ngDevMode&&assertNodeType(element,3),ngDevMode&&assertNotEqual(element.data,null,"isComponent"),ngDevMode&&assertDataInRange(directiveIndex);var directive=getDirectiveInstance(data[directiveIndex]),hostView=element.data;ngDevMode&&assertNotEqual(hostView,null,"hostView");var oldView=enterView(hostView,element);try{template(directive,creationMode)}finally{refreshDynamicChildren(),leaveView(oldView)}}}function addToViewTree(state){return currentView.tail?currentView.tail.next=state:currentView.child=state,currentView.tail=state,state}function bindV(values){var different,parts;if(different=creationMode)"number"!=typeof currentView.bindingStartIndex&&(bindingIndex=currentView.bindingStartIndex=data.length),data[bindingIndex++]=parts=values.slice();else{parts=data[bindingIndex++],different=!1;for(var i=0;i<values.length;i++)(different=different||values[i]!==NO_CHANGE&&isDifferent(values[i],parts[i]))&&values[i]!==NO_CHANGE&&(parts[i]=values[i])}if(different){for(var str=stringify$1(parts[0]),i=1;i<parts.length;i++)str+=stringify$1(parts[i]);return str}return NO_CHANGE}function bind(value){var different;return(different=creationMode)?("number"!=typeof currentView.bindingStartIndex&&(bindingIndex=currentView.bindingStartIndex=data.length),data[bindingIndex++]=value):((different=value!==NO_CHANGE&&isDifferent(data[bindingIndex],value))&&(data[bindingIndex]=value),bindingIndex++),different?value:NO_CHANGE}function bind1(prefix,value,suffix){return bind(value)===NO_CHANGE?NO_CHANGE:prefix+stringify$1(value)+suffix}function bind2(prefix,v0,i0,v1,suffix){var different;if(different=creationMode)"number"!=typeof currentView.bindingStartIndex&&(bindingIndex=currentView.bindingStartIndex=data.length),data[bindingIndex++]=v0,data[bindingIndex++]=v1;else{var part0=data[bindingIndex++],part1=data[bindingIndex++];v0===NO_CHANGE&&(v0=part0),v1===NO_CHANGE&&(v1=part1),(different=isDifferent(part0,v0)||isDifferent(part1,v1))&&(data[bindingIndex-2]=v0,data[bindingIndex-1]=v1)}return different?prefix+stringify$1(v0)+i0+stringify$1(v1)+suffix:NO_CHANGE}function bind3(prefix,v0,i0,v1,i1,v2,suffix){var different;if(different=creationMode)"number"!=typeof currentView.bindingStartIndex&&(bindingIndex=currentView.bindingStartIndex=data.length),data[bindingIndex++]=v0,data[bindingIndex++]=v1,data[bindingIndex++]=v2;else{var part0=data[bindingIndex++],part1=data[bindingIndex++],part2=data[bindingIndex++];v0===NO_CHANGE&&(v0=part0),v1===NO_CHANGE&&(v1=part1),v2===NO_CHANGE&&(v2=part2),(different=isDifferent(part0,v0)||isDifferent(part1,v1)||isDifferent(part2,v2))&&(data[bindingIndex-3]=v0,data[bindingIndex-2]=v1,data[bindingIndex-1]=v2)}return different?prefix+stringify$1(v0)+i0+stringify$1(v1)+i1+stringify$1(v2)+suffix:NO_CHANGE}function bind4(prefix,v0,i0,v1,i1,v2,i2,v3,suffix){var different;if(different=creationMode)"number"!=typeof currentView.bindingStartIndex&&(bindingIndex=currentView.bindingStartIndex=data.length),data[bindingIndex++]=v0,data[bindingIndex++]=v1,data[bindingIndex++]=v2,data[bindingIndex++]=v3;else{var part0=data[bindingIndex++],part1=data[bindingIndex++],part2=data[bindingIndex++],part3=data[bindingIndex++];v0===NO_CHANGE&&(v0=part0),v1===NO_CHANGE&&(v1=part1),v2===NO_CHANGE&&(v2=part2),v3===NO_CHANGE&&(v3=part3),(different=isDifferent(part0,v0)||isDifferent(part1,v1)||isDifferent(part2,v2)||isDifferent(part3,v3))&&(data[bindingIndex-4]=v0,data[bindingIndex-3]=v1,data[bindingIndex-2]=v2,data[bindingIndex-1]=v3)}return different?prefix+stringify$1(v0)+i0+stringify$1(v1)+i1+stringify$1(v2)+i2+stringify$1(v3)+suffix:NO_CHANGE}function bind5(prefix,v0,i0,v1,i1,v2,i2,v3,i3,v4,suffix){var different;if(different=creationMode)"number"!=typeof currentView.bindingStartIndex&&(bindingIndex=currentView.bindingStartIndex=data.length),data[bindingIndex++]=v0,data[bindingIndex++]=v1,data[bindingIndex++]=v2,data[bindingIndex++]=v3,data[bindingIndex++]=v4;else{var part0=data[bindingIndex++],part1=data[bindingIndex++],part2=data[bindingIndex++],part3=data[bindingIndex++],part4=data[bindingIndex++];v0===NO_CHANGE&&(v0=part0),v1===NO_CHANGE&&(v1=part1),v2===NO_CHANGE&&(v2=part2),v3===NO_CHANGE&&(v3=part3),v4===NO_CHANGE&&(v4=part4),(different=isDifferent(part0,v0)||isDifferent(part1,v1)||isDifferent(part2,v2)||isDifferent(part3,v3)||isDifferent(part4,v4))&&(data[bindingIndex-5]=v0,data[bindingIndex-4]=v1,data[bindingIndex-3]=v2,data[bindingIndex-2]=v3,data[bindingIndex-1]=v4)}return different?prefix+stringify$1(v0)+i0+stringify$1(v1)+i1+stringify$1(v2)+i2+stringify$1(v3)+i3+stringify$1(v4)+suffix:NO_CHANGE}function bind6(prefix,v0,i0,v1,i1,v2,i2,v3,i3,v4,i4,v5,suffix){var different;if(different=creationMode)"number"!=typeof currentView.bindingStartIndex&&(bindingIndex=currentView.bindingStartIndex=data.length),data[bindingIndex++]=v0,data[bindingIndex++]=v1,data[bindingIndex++]=v2,data[bindingIndex++]=v3,data[bindingIndex++]=v4,data[bindingIndex++]=v5;else{var part0=data[bindingIndex++],part1=data[bindingIndex++],part2=data[bindingIndex++],part3=data[bindingIndex++],part4=data[bindingIndex++],part5=data[bindingIndex++];v0===NO_CHANGE&&(v0=part0),v1===NO_CHANGE&&(v1=part1),v2===NO_CHANGE&&(v2=part2),v3===NO_CHANGE&&(v3=part3),v4===NO_CHANGE&&(v4=part4),v5===NO_CHANGE&&(v5=part5),(different=isDifferent(part0,v0)||isDifferent(part1,v1)||isDifferent(part2,v2)||isDifferent(part3,v3)||isDifferent(part4,v4)||isDifferent(part5,v5))&&(data[bindingIndex-6]=v0,data[bindingIndex-5]=v1,data[bindingIndex-4]=v2,data[bindingIndex-3]=v3,data[bindingIndex-2]=v4,data[bindingIndex-1]=v5)}return different?prefix+stringify$1(v0)+i0+stringify$1(v1)+i1+stringify$1(v2)+i2+stringify$1(v3)+i3+stringify$1(v4)+i4+stringify$1(v5)+suffix:NO_CHANGE}function bind7(prefix,v0,i0,v1,i1,v2,i2,v3,i3,v4,i4,v5,i5,v6,suffix){var different;if(different=creationMode)"number"!=typeof currentView.bindingStartIndex&&(bindingIndex=currentView.bindingStartIndex=data.length),data[bindingIndex++]=v0,data[bindingIndex++]=v1,data[bindingIndex++]=v2,data[bindingIndex++]=v3,data[bindingIndex++]=v4,data[bindingIndex++]=v5,data[bindingIndex++]=v6;else{var part0=data[bindingIndex++],part1=data[bindingIndex++],part2=data[bindingIndex++],part3=data[bindingIndex++],part4=data[bindingIndex++],part5=data[bindingIndex++],part6=data[bindingIndex++];v0===NO_CHANGE&&(v0=part0),v1===NO_CHANGE&&(v1=part1),v2===NO_CHANGE&&(v2=part2),v3===NO_CHANGE&&(v3=part3),v4===NO_CHANGE&&(v4=part4),v5===NO_CHANGE&&(v5=part5),v6===NO_CHANGE&&(v6=part6),(different=isDifferent(part0,v0)||isDifferent(part1,v1)||isDifferent(part2,v2)||isDifferent(part3,v3)||isDifferent(part4,v4)||isDifferent(part5,v5)||isDifferent(part6,v6))&&(data[bindingIndex-7]=v0,data[bindingIndex-6]=v1,data[bindingIndex-5]=v2,data[bindingIndex-4]=v3,data[bindingIndex-3]=v4,data[bindingIndex-2]=v5,data[bindingIndex-1]=v6)}return different?prefix+stringify$1(v0)+i0+stringify$1(v1)+i1+stringify$1(v2)+i2+stringify$1(v3)+i3+stringify$1(v4)+i4+stringify$1(v5)+i5+stringify$1(v6)+suffix:NO_CHANGE}function bind8(prefix,v0,i0,v1,i1,v2,i2,v3,i3,v4,i4,v5,i5,v6,i6,v7,suffix){var different;if(different=creationMode)"number"!=typeof currentView.bindingStartIndex&&(bindingIndex=currentView.bindingStartIndex=data.length),data[bindingIndex++]=v0,data[bindingIndex++]=v1,data[bindingIndex++]=v2,data[bindingIndex++]=v3,data[bindingIndex++]=v4,data[bindingIndex++]=v5,data[bindingIndex++]=v6,data[bindingIndex++]=v7;else{var part0=data[bindingIndex++],part1=data[bindingIndex++],part2=data[bindingIndex++],part3=data[bindingIndex++],part4=data[bindingIndex++],part5=data[bindingIndex++],part6=data[bindingIndex++],part7=data[bindingIndex++];v0===NO_CHANGE&&(v0=part0),v1===NO_CHANGE&&(v1=part1),v2===NO_CHANGE&&(v2=part2),v3===NO_CHANGE&&(v3=part3),v4===NO_CHANGE&&(v4=part4),v5===NO_CHANGE&&(v5=part5),v6===NO_CHANGE&&(v6=part6),v7===NO_CHANGE&&(v7=part7),(different=isDifferent(part0,v0)||isDifferent(part1,v1)||isDifferent(part2,v2)||isDifferent(part3,v3)||isDifferent(part4,v4)||isDifferent(part5,v5)||isDifferent(part6,v6)||isDifferent(part7,v7))&&(data[bindingIndex-8]=v0,data[bindingIndex-7]=v1,data[bindingIndex-6]=v2,data[bindingIndex-5]=v3,data[bindingIndex-4]=v4,data[bindingIndex-3]=v5,data[bindingIndex-2]=v6,data[bindingIndex-1]=v7)}return different?prefix+stringify$1(v0)+i0+stringify$1(v1)+i1+stringify$1(v2)+i2+stringify$1(v3)+i3+stringify$1(v4)+i4+stringify$1(v5)+i5+stringify$1(v6)+i6+stringify$1(v7)+suffix:NO_CHANGE}function getRenderer(){return renderer}function getDirectiveInstance(instanceOrArray){return Array.isArray(instanceOrArray)?instanceOrArray[0]:instanceOrArray}function assertPreviousIsParent(){assertEqual(isParent,!0,"isParent")}function assertHasParent(){assertNotEqual(previousOrParentNode.parent,null,"isParent")}function assertDataInRange(index,arr){null==arr&&(arr=data),assertLessThan(index,arr?arr.length:0,"data.length")}function assertDataNext(index){assertEqual(data.length,index,"data.length not in sequence")}/**
 * @license
 * Copyright Google Inc. All Rights Reserved.
 *
 * Use of this source code is governed by an MIT-style license that can be
 * found in the LICENSE file at https://angular.io/license
 */
function renderComponent(componentType,opts){void 0===opts&&(opts={});var rendererFactory=opts.rendererFactory||domRendererFactory3,componentDef=componentType.ngComponentDef;componentDef.type!=componentType&&(componentDef.type=componentType);var component,hostNode=locateHostElement(rendererFactory,opts.host||componentDef.tag),oldView=enterView(createLView(-1,rendererFactory.createRenderer(hostNode,componentDef.rendererType),createTView()),null);try{hostElement(hostNode,componentDef),component=getDirectiveInstance(directiveCreate(1,componentDef.n(),componentDef))}finally{leaveView(oldView)}return opts.features&&opts.features.forEach(function(feature){return feature(component,componentDef)}),detectChanges(component),component}function detectChanges(component){ngDevMode&&assertNotNull(component,"component");var hostNode=component[NG_HOST_SYMBOL];ngDevMode&&!hostNode&&createError("Not a directive instance",component),ngDevMode&&assertNotNull(hostNode.data,"hostNode.data"),renderComponentOrTemplate(hostNode,hostNode.view,component),isDirty=!1}function getOrCreateElementRef(di){return di.elementRef||(di.elementRef=new ElementRef$1(0==(3&di.node.flags)?null:di.node.native))}function getOrCreateContainerRef(di){return di.viewContainerRef||(di.viewContainerRef=new ViewContainerRef$1(di.node))}function getOrCreateTemplateRef(di){ngDevMode&&assertNodeType(di.node,0);var data=di.node.data;return di.templateRef||(di.templateRef=new TemplateRef$1(getOrCreateElementRef(di),data.template,getRenderer()))}/**
 * @license
 * Copyright Google Inc. All Rights Reserved.
 *
 * Use of this source code is governed by an MIT-style license that can be
